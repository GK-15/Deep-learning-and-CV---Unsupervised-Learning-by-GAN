...
class Generator(nn.Module):
    ...
    def __init__(self, input_dims=100, output_dims=784):
        """Init function
        ...
        self.fc1 = nn.Sequential(nn.Linear(input_dims, 256), nn.LeakyReLU())
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_dims=784, output_dims=1):
        """Init function
        ...
        ###  TODO: Change the architecture and value as CW2 Guidance required
        self.fc1 = nn.Sequential(nn.Linear(input_dims, 1024), nn.LeakyReLU(), nn.Dropout(0.3))
...
def show_result(G_net, z_, num_epoch, show=False, save=False, path='result.png'):
    ...
    # hint: use plt.imshow and plt.savefig to display and store the image
    z_ = torch.randn(25, 100)
    test_image_data = G_net(z_)
    grid_axis_size = 5
    fig,axis = plt.subplots(grid_axis_size, grid_axis_size, figsize=(5, 5))

    for x, y in itertools.product(range(grid_axis_size), range(grid_axis_size)):
        axis[x, y].get_xaxis().set_visible(False)
        axis[x, y].get_yaxis().set_visible(False)

    for generate_mnist_data in range(grid_axis_size * grid_axis_size):
        x = (generate_mnist_data // grid_axis_size)
        y = (generate_mnist_data % grid_axis_size)
        axis[x, y].cla()
        axis[x, y].imshow(test_image_data[generate_mnist_data].cpu().data.view(28, 28).numpy(), cmap='gray')

    fig.text(0.5, 0.05, 'Epoch %s'%num_epoch, horizontalalignment='center')
    if save:
        plt.savefig(path)
    if show:
        plt.show()
